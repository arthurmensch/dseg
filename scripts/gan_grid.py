...
job_directory = expanduser('~/output/games_rl/jobs_seed')
...
def write_header(i, f):
    ...
    f.writelines(f"#SBATCH --job-name=seed_gan\n")
...
parameters = ParameterGrid({'lr': [5e-4, 8e-4],
                            'G_lr_factor': [.1], 'sampling': ['all', 'alternated'],
                            'seed': [1100, 1200, 1300, 1400]})
i = 0
for p in parameters:
